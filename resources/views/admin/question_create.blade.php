...
@section('content')
    <div class="col-md-12">
        <div class="card">
            <div class="card-header"><img class="title_ic" src="images/yukidaruma.png"><span class="title"><span>Ｑ</span>ｕｅｓｔｉｏｎ - 新規作成</span></div>
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.question_check') }}">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <div class="col-md-8">
                                <select id="level" class="form-control @error('level') is-invalid @enderror" name="level" required autocomplete="level">
                                    ...
                                    <option value="{{ $level->name }}" @if(old('level') == $level->name) selected @endif>{{ $level->name }}</option>
                                    @endforeach
                                </select>
                    ...
                    <!-- 回答タイプ -->
                    <input type="hidden" id="typeValue" value="{{ old('type') }}">
                    <div class="form-group">
                        ...
                        <div class="col-md-4 offset-md-2">
                            <select id="type" class="form-control @error('type') is-invalid @enderror" name="type" required autocomplete="type" onChange="changeType()">
                                ...
                                <option value="{{ $type->name }}" @if(old('type') == $type->name) selected @endif>{{ $type->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-8 offset-md-2">
                            <textarea id="commentary" class="form-control" name="commentary" autocomplate="commentary">{{ old('commentary') }}</textarea>
                        </div>
                    </div>
